...
const MobileHeader: React.FC<MobileHeaderProps> = ({
  ...
  const socialicons: { link: string; icon: JSX.Element; title: string }[] = [];

  // Ritual — governance/developers/community menus disabled until real links exist.
              ...
              {govermance.map((item, index) => (
                ...
                >
                  <Link
                    ...
                    to={item.link}
                    style={{ color: '#c7cad9', textDecoration: 'none' }}
                  >
                    {item.text}
                  </Link>
                </Box>
