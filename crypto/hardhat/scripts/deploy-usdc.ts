...
const artifact = JSON.parse(
  fs.readFileSync("./artifacts/contracts/USDC.sol/USDC.json", "utf8"),
);
...
// Optional override: USDC_OWNER env var (defaults to deployer).
const owner = (process.env.USDC_OWNER ?? walletClient.account.address) as `0x${string}`;
...
console.log("USDC deployed at:", receipt.contractAddress);
